...
module.exports = exports = {
    //判断是否是授权用户
    has: function(nick) {
        return data.jobSetting.some(function(item){
    ...
    getJob: function(id){
        ...
        //nick（昵称）
            ...
            for(var i=0; i< data.jobSetting.length; ++i){
                if(data.jobSetting[i].marker.join().indexOf(id) >= 0){
                    return arguments.callee.call(this,data.jobSetting[i].jobId);
                }
            }
        }
    },
    //取得岗位配置信息
    getJobSetting: function(id){
        for(var i=0; i < data.jobSetting.length; ++i){
            if(data.jobSetting[i].jobId === id){
                return data.jobSetting[i];
            }
        }
    },
    //根据岗位分配下一个阅卷人
    getNextMarker: function(jobId){
        var nextMarker;
        var setting = exports.getJobSetting(jobId);
        if(setting){
            ...
            if(!global.nextMarkers){
                ...
                for(var i=0; i < data.jobSetting.length; ++i){
                    var item = {};
                    item.jobId = data.jobSetting[i].id;
                    item.marker = data.jobSetting[i].marker[0] || "";
                    global.nextMarkers.push(item);
                }
            }

            for(var i=0; i<global.nextMarkers.length; ++i){
                if(global.nextMarkers[i].jobId == jobId){
